...
void makecoupling(double t, double *y, double *f, void* pars){
  ...
  if(p->A){
    ...
    double alpha=1;
    double beta=0;
    cublasDgemv(p->handle, CUBLAS_OP_T, p->N, p->N, &alpha, p->adj, p->N, p->y2, 2, &beta, p->f2, 2);
    cublasDgemv(p->handle, CUBLAS_OP_T, p->N, p->N, &alpha, p->adj, p->N, (p->y2)+1, 2, &beta, (p->f2)+1, 2);
  }
}
...
int main (int argc, char* argv[]) {
    ...
    double *omegasloc, *omegas, *yloc, *adjloc, *adj, *y2, *f2, *f3, *ones;
